...
public:
	...
	LinkedList(std::initializer_list<T> li);
	...
	T& search(const std::function<bool(const T& a)>& cmp);
	const T& search(const std::function<bool(const T& a)>& cmp) const;
	LinkedList<T&> searchAll(const std::function<bool(const T& a)>& cmp);
	LinkedList<const T&> searchAll(const std::function<bool(const T& a)>& cmp) const;
};
