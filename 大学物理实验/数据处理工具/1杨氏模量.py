"""
这是拉伸法测量金属丝的杨氏模量的计算程序，大家只需要把函数的实参和变量的数据替换成自己的就可以啦。
...
if __name__ == "__main__":
    ...
    d0 = -0.002  # 螺旋测微器零差
    Di = [0.635, 0.641, 0.730, 0.640, 0.642, 0.637]  # 金属丝直径 d 六次测量值、
    bi = [44.70, 44.65, 44.75, 44.68, 44.69, 44.72]  # 水平卡座 b 长度六次测量值
    D = sum(Di) / len(Di)  # 平均值
    B = sum(bi) / len(bi)  # 平均值

    # 表 4-3 加减力时标尺刻度与对应拉力的数据
    n_up = [
        1.10,
        1.30,
        1.70,
    ...
    ni = [(n_up[i] + n_down[i]) / 2 for i in range(len(n_up))]  # 平均标尺刻度
    Ni = [(ni[i + 6] - ni[i]) for i in range(6)]  # 逐差法
    N = sum(Ni) / len(Ni)  # 平均值
    # ============================ 数据代入区结束 =============================
