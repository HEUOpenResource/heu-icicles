...
static int none_of(NFAState **arr, NFAState *state, int count)
{
	for (int i = 0; i < count; ++i)
	{
		if (arr[i]->Name == state->Name)
		{
			return 0;
		}
	}
...
int NFAStateIsSubset(DFA *pDFA, Transform *pTransform)
{
	...
	{
		DFAState *pDFAState = pDFA->DFAlist[i];
		int find = 1;
		for (int j = 0; j < pTransform->NFAStateCount; ++j)
		{
			if (none_of(pDFAState->NFAlist, pTransform->NFAlist[j], pDFAState->NFAStateCount))
			{
				find = 0;
				break;
			}
...
{
	Transform *tf = pDFAState->firstTran;
	while (tf)
	{
		if (tf->TransformChar == TransformChar)
		{
			return tf;
		}
		tf = tf->NextTrans;
...
void AddNFAStateArrayToTransform(NFAState **NFAStateArray, int Count, Transform *pTransform)
{
	for (int i = 0; i < Count; ++i)
	{
		if (none_of(pTransform->NFAlist, NFAStateArray[i], pTransform->NFAStateCount))
		{
...
void Closure(NFAState *State, NFAState **StateArray, int *Count)
{
	...
	*Count = 0;
	...
	while (0 == NFAStateStackEmpty(&StateStack))
	{
		NFAState *st = PopNFAState(&StateStack);
		while (st)
		{
			StateArray[(*Count)++] = st;
			if (st->Transform == VoidTrans)
			{
				if (st->Next2)
				{
					if (none_of(StateArray, st->Next2, *Count))
					{
						PushNFAState(&StateStack, st->Next2);
					}
				}
				if (none_of(StateArray, st->Next1, *Count))
				{
					st = st->Next1;
				}
				else
	...
	for (i = 0; i < pDFA->length; i++)
	{
		...
		for (j = 0; j < pDFA->DFAlist[i]->NFAStateCount; j++)
		{
			...
			if (pTransform == NULL)
			{
				...
				Transform *oldTF = pDFA->DFAlist[i]->firstTran;
				pDFA->DFAlist[i]->firstTran = CreateDFATransform(NFAStateTemp->Transform, NFAStateArray, Count);
				pDFA->DFAlist[i]->firstTran->NextTrans = oldTF;
			}
			else
			{
...
void FreeDFA(DFA *pDFA)
{
	for (int i = 0; i < pDFA->length; ++i)
	{
		while (pDFA->DFAlist[i]->firstTran)
		{
			Transform *tf = pDFA->DFAlist[i]->firstTran;
			pDFA->DFAlist[i]->firstTran = tf->NextTrans;
			free(tf);
		}
		free(pDFA->DFAlist[i]);
	}
